...
DRAMA_CAP_MAP = {
    'chen_mo_de_zhen_xiang': 'chen_mo_de_zhen_xiang_hierarchical_caption_Hiera_1027_3API_briefadaptlength_20251230.json',     
    ...
    'downton_abbey': 'downton_abbey_hierarchical_caption_Hiera_0202_3API_briefadaptlength_20260202.json',            
...
class LLMInference:
    ...
    def process_item(self, key, q_info):
        ...
        for idx in selected_indices:
            ...
            #     msg[1]["content"].append(
            #         {"type": "text", "text": "这是上面一张图的说话人信息：{}:{}".format(speaker_info[speaker_key]["role"],
            #                                                                          speaker_info[speaker_key]["text"])})

        result, total_tokens = self.get_llm_response(msg, key)
        return key, {'result': result, 'total_tokens': total_tokens}
    
    def get_llm_response(self, msg, key, max_retries=5, delay_base=0.5):
        for attempt in range(1, max_retries + 1):
            ...
            if match:
                return match.group(1), total_tokens # 提取成功，立即返回
    ...
    def find_containing_source(self, source_times, target_time):
        ...
        for seg in source_times:
            ...
            if src_start <= tgt_start and tgt_end <= src_end:
                return seg
    ...
    def sample_frames_from_video(self, time, save_path):
        # 1. 解析开始和结束时间字符串
        ...
        for sample_item in sample_res:
            ...
            for im_id in range(images.shape[0]):
                ...
                if 'S' in save_render_path.split('/')[-2]:
                    success = cv2.imwrite(os.path.join(save_render_path, 
                                                    season + '_' + episode + '_' + str(frame_id)+".jpg"), 
                                                    frame, 
                                                    [cv2.IMWRITE_JPEG_QUALITY, 100])
                else:
                    success = cv2.imwrite(os.path.join(save_render_path, 
                                                    episode + '_' + str(frame_id)+".jpg"), 
                                                    frame, 
                                                    [cv2.IMWRITE_JPEG_QUALITY, 100])
    ...
    def get_speaker(self, time, sample_res):
        speaker_save_info = {}
        for sample_item in sample_res:
    ...
    def evaluation(self, result):
        ...
        for q_i, q_info in result.items():
            an = q_info['Q&A']['Answer'].split(' ')[0][:1]
            try:
                assert an in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']
            except AssertionError:
                import pdb; pdb.set_trace()
            options = q_info['Q&A']['Options'].split('\n')
            options = [option for option in options if option.strip() != '']
            if 'result' not in q_info['Q&A'].keys() or q_info['Q&A']['result'] not in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']:
                q_info['Q&A']['result'] = random.choice(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'][:len(options)])
        ...
        print('{}, {}, {}, {}, question num: {}, accuracy: {:.2f}'.format(
                                                                    self.drama,
                                                                    self.sample_frames,
                                                                    self.level,
